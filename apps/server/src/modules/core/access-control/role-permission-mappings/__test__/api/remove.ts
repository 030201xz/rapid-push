 ...
 * 2. 创建测试角色和测试权限
 * 3. 为角色分配权限
 * 4. 调用 remove API 移除映射
 * 5. 验证移除成功
 * 6. 清理测试数据
 *
 * 运行: bun run src/modules/core/access-control/role-permission-mappings/__test__/api/remove.ts
 */
...
const logger = createLogger({
  namespace: 'Test:RolePermissionMappings:Remove',
});
...
const NEW_PERMISSION = {
  code: `test_permission_remove_${Date.now()}`,
  name: '测试权限（移除映射）',
  description: '用于测试移除映射的权限',
  type: 'api' as const,
  resource: '/api/test/remove',
  sortPriority: 100,
  isActive: true,
};
...
/** 测试移除映射 */
async function testRemoveMapping(client: Client, mappingId: string) {
  logger.info('测试移除映射...', { mappingId });

  const mappingsApi = getMappingsApi(client);
  await mappingsApi.remove.mutate({ id: mappingId });

  logger.info('映射移除成功', { mappingId });
}
...
async function main() {
  logger.info('开始移除映射 API 测试', { apiUrl: API_URL });
  ...
  try {
    ...
    // 4. 为角色分配权限
    const mapping = await assignPermission(
      authedClient,
      testRoleId,
      testPermissionId
    );
    ...
    // 5. 移除映射
    await testRemoveMapping(authedClient, mapping.id);
