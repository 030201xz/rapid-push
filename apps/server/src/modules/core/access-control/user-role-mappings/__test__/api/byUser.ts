 ...
 * 2. 获取用户列表，取一个用户的 ID
 * 3. 调用 byUser API 获取该用户的所有角色
 * 4. 验证返回结果
 *
 * 运行: bun run src/modules/core/access-control/user-role-mappings/__test__/api/byUser.ts
 */
...
const logger = createLogger({
  namespace: 'Test:UserRoleMappings:ByUser',
});
...
/** 测试获取用户的所有角色 */
async function testGetUserRoles(client: Client) {
  logger.info('测试获取用户的所有角色...');
  ...
  // 获取该用户的所有角色
  const userRoles = await mappingsApi.byUser.query({
...
async function main() {
  logger.info('开始获取用户角色 API 测试', { apiUrl: API_URL });
  ...
  try {
    ...
    // 3. 获取用户角色
    await testGetUserRoles(authedClient);
