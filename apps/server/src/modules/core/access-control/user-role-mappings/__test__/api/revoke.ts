 ...
 * 2. 创建测试用户和测试角色
 * 3. 分配角色给用户
 * 4. 调用 revoke API 撤销映射
 * 5. 验证撤销成功
 * 6. 清理测试数据
 *
 * 运行: bun run src/modules/core/access-control/user-role-mappings/__test__/api/revoke.ts
 */
...
const logger = createLogger({
  namespace: 'Test:UserRoleMappings:Revoke',
});
...
// 待创建的测试用户
const NEW_USER = {
  username: `test_user_revoke_${timestamp}`,
  email: `test_user_revoke_${timestamp}@test.com`,
  nickname: '测试用户（撤销映射）',
  passwordHash: '',
};
...
// 待创建的测试角色
const NEW_ROLE = {
  code: `test_role_revoke_${timestamp}`,
  name: '测试角色（撤销映射）',
  description: '用于测试撤销映射的角色',
  level: 10,
  isSystem: false,
  isActive: true,
};
...
/** 测试撤销映射 */
async function testRevokeMapping(client: Client, mappingId: string) {
  logger.info('测试撤销映射...', { mappingId });

  const mappingsApi = getMappingsApi(client);
  await mappingsApi.revoke.mutate({ id: mappingId });

  logger.info('映射撤销成功', { mappingId });
}
...
async function main() {
  logger.info('开始撤销映射 API 测试', { apiUrl: API_URL });
  ...
  try {
    ...
    // 4. 分配角色给用户
    const mapping = await assignRole(
      authedClient,
      testUserId,
      testRoleId
    );
    ...
    // 5. 撤销映射
    await testRevokeMapping(authedClient, mapping.id);
