 ...
 * - 验证更新记录和资源已创建
 *
 * 运行: bun run src/modules/hot-update/__test__/api/00-basic-workflow/01-upload.ts
 */
...
async function main() {
  ...
  logger.info('📦 基础工作流 - 步骤 01: 上传热更新 Bundle');
  logger.info('='.repeat(50));
  ...
  try {
    ...
    formData.append(
      'extra',
      JSON.stringify({ testEnv: true, createdBy: 'api-test' })
    );
    ...
    logger.info('✅ 更新创建成功', { id: updateId, assetCount });
    ...
    logger.info('='.repeat(50));
    logger.info('下一步: bun run .../00-basic-workflow/02-check.ts');
  } catch (error) {
