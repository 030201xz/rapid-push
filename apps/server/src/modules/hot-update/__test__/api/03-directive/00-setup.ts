 ...
 * 指令场景 - 步骤 00: 环境准备
 *
 * 运行: bun run src/modules/hot-update/__test__/api/03-directive/00-setup.ts
 */
...
async function main() {
  ...
  try {
    ...
    logger.info('='.repeat(50));
    logger.info(
      '下一步: bun run .../03-directive/01-create-rollback-directive.ts'
    );
  } catch (error) {
