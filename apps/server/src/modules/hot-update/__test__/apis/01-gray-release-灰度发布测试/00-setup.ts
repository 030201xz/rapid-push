 ...
 * - 创建测试渠道
 *
 * 运行: bun run src/modules/hot-update/__test__/apis/01-gray-release-灰度发布测试/00-setup.ts
 */
...
const logger = createTestLogger('GrayRelease:00-Setup');

async function main() {
  logger.info('='.repeat(60));
  logger.info('🎯 灰度发布 - 步骤 00: 初始化测试环境');
  logger.info('='.repeat(60));
  ...
  try {
    ...
    try {
      const org = await manage.organizations.create.mutate({
        name: 'Gray Release Test Org',
        slug: 'gray-test',
        description: '灰度发布测试组织',
      });
    ...
    try {
      const project = await manage.projects.create.mutate({
        organizationId,
        name: 'Gray Release Test App',
        slug: 'gray-app',
        description: '灰度发布测试应用',
      });
    ...
    try {
      const channel = await manage.channels.create.mutate({
        projectId,
        name: 'gray-test-channel',
        description: '灰度发布测试渠道',
      });
