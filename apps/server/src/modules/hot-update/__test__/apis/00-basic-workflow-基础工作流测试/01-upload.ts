 ...
 * - 验证上传成功
 *
 * 运行: bun run src/modules/hot-update/__test__/apis/00-basic-workflow-基础工作流测试/01-upload.ts
 */
...
const logger = createTestLogger('BasicWorkflow:01-Upload');

/** 创建最小化测试 Bundle ZIP */
async function createTestBundleZip(): Promise<Buffer> {
  const tmpDir = '/tmp/rapid-s-basic-test';
  const zipPath = '/tmp/rapid-s-basic-test.zip';
  ...
  const bundleContent = `
// Basic Test Bundle
console.log('Basic test bundle loaded');
export default { version: '1.0.0', message: 'Hello World' };
  `.trim();
...
async function main() {
  ...
  try {
    ...
    formData.append('description', '基础工作流测试更新');
