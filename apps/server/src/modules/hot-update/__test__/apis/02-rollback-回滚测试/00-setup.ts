 ...
 * - 创建测试渠道
 *
 * 运行: bun run src/modules/hot-update/__test__/apis/02-rollback-回滚测试/00-setup.ts
 */
...
async function main() {
  ...
  logger.info('🎯 回滚测试 - 步骤 00: 初始化测试环境');
  logger.info('='.repeat(60));
    ...
    try {
      ...
      if (
        ...
        const channel = channels.find(
          c => c.name === 'rollback-test-channel'
        );
