 ...
 * - 清除测试上下文
 *
 * 运行: bun run src/modules/hot-update/__test__/apis/02-rollback-回滚测试/99-cleanup.ts
 */
...
async function main() {
  ...
  try {
    ...
    logger.info(`Directive ID: ${ctx.rollbackDirectiveId ?? '(无)'}`);
