 ...
 * 2. 创建测试组织、项目和渠道
 * 3. 调用 byId 根据 ID 获取渠道详情
 * 4. 验证返回结果正确
 * 5. 测试查询不存在的渠道
 * 6. 通过 API 清理测试数据
 *
 * 运行: bun run src/modules/hot-update/channels/__test__/api/byId.ts
 */
...
// ========== Logger 实例 ==========
const logger = createLogger({ namespace: 'Test:ChannelById' });
...
const TEST_PROJECT = {
  ...
  description: '用于渠道 byId 测试的项目',
};

// 测试渠道
const TEST_CHANNEL = {
  name: 'production',
  description: '用于 byId 测试的渠道',
};
...
/** 测试根据 ID 获取渠道详情 */
async function testGetChannelById(
  accessToken: string,
  channelId: string,
  expectedProjectId: string
) {
  logger.info('测试根据 ID 获取渠道详情...', { channelId });
  ...
  if (channel.id !== channelId) {
    throw new Error(
      `返回的渠道 ID 不匹配: ${channel.id} !== ${channelId}`
    );
  }
  ...
  if (channel.projectId !== expectedProjectId) {
    throw new Error(`返回的项目 ID 不匹配`);
  }

  if (!channel.channelKey) {
    throw new Error(`返回的 channelKey 不存在`);
  }
  ...
  logger.info('渠道详情获取成功', {
    ...
    signingEnabled: channel.signingEnabled,
...
/** 测试查询不存在的渠道 */
async function testGetNonExistentChannel(accessToken: string) {
  logger.info('测试查询不存在的渠道...');
  ...
  const channel = await channelsApi.byId.query({ id: nonExistentId });
...
async function main() {
  ...
  try {
    ...
    // 6. 测试查询不存在的渠道
    await testGetNonExistentChannel(accessToken);
