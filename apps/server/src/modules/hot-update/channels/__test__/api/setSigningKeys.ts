...
async function testGetPublicKey(
  ...
  if (!result.publicKey.includes('BEGIN PUBLIC KEY')) {
...
async function testUpdateSigningKeys(
  ...
  // 验证公钥已更新
  const publicKeyResult = await channelsApi.getPublicKey.query({
    id: channelId,
  });

  if (publicKeyResult?.publicKey !== TEST_SIGNING_KEYS_2.publicKey) {
