 ...
 * 2. 创建测试组织
 * 3. 调用 isMember API 验证返回数据
 * 4. 清理测试数据
 *
 * 运行: bun run src/modules/hot-update/organization-members/__test__/api/isMember.ts
 */
...
const logger = createLogger({
  namespace: 'Test:OrgMembers:IsMember',
});
...
/** 测试检查是否为组织成员 */
async function testIsMember(
  accessToken: string,
  organizationId: string
) {
  logger.info('测试检查是否为组织成员...');

  const authedClient = createClient(API_URL, { token: accessToken });
  const membersApi = getMembersApi(authedClient);

  const isMember = await membersApi.isMember.query({
    organizationId,
  });

  logger.info('检查成员资格成功', {
    organizationId,
    isMember,
  });
...
async function main() {
  logger.info('开始 isMember API 测试', { apiUrl: API_URL });
  ...
  try {
    ...
    // 3. 测试检查是否为组织成员
    const isMember = await testIsMember(accessToken, testOrgId);

    // 验证：创建者应该是成员（如果自动添加的话）
    logger.info('成员资格检查结果', { isMember });
