 ...
 * 6. 清理测试数据
 *
 * 运行: bun run src/modules/hot-update/organization-members/__test__/api/add.ts
 */
...
// ========== Logger 实例 ==========
const logger = createLogger({ namespace: 'Test:OrgMembers:Add' });
...
/** 验证成员已添加 */
async function verifyMemberAdded(
  accessToken: string,
  organizationId: string
) {
  logger.info('验证成员已添加...');
...
async function main() {
  logger.info('开始 add API 测试', { apiUrl: API_URL });
  ...
  try {
    ...
    // 4. 添加成员
    await testAddMember(accessToken, testOrgId, testUserId);

    // 5. 验证成员已添加
    await verifyMemberAdded(accessToken, testOrgId);
