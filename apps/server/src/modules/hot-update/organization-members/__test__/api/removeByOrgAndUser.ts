 ...
 * 3. 创建测试用户
 * 4. 添加测试成员
 * 5. 调用 removeByOrgAndUser API 移除成员
 * 6. 验证成员已移除
 * 7. 清理测试数据
 *
 * 运行: bun run src/modules/hot-update/organization-members/__test__/api/removeByOrgAndUser.ts
 */
...
const logger = createLogger({
  namespace: 'Test:OrgMembers:RemoveByOrgAndUser',
});
...
/** 测试通过组织ID和用户ID移除成员 */
async function testRemoveByOrgAndUser(
  accessToken: string,
  organizationId: string,
  userId: string
) {
  logger.info('测试通过组织ID和用户ID移除成员...');

  const authedClient = createClient(API_URL, { token: accessToken });
  const membersApi = getMembersApi(authedClient);

  const result = await membersApi.removeByOrgAndUser.mutate({
    organizationId,
    userId,
  });

  logger.info('通过组织ID和用户ID移除成员成功', {
    organizationId,
    userId,
    result,
  });
...
async function main() {
  logger.info('开始 removeByOrgAndUser API 测试', {
    apiUrl: API_URL,
  });
  ...
  try {
    ...
    // 4. 添加成员
    await addMember(accessToken, testOrgId, testUserId);

    // 5. 测试通过组织ID和用户ID移除成员
    await testRemoveByOrgAndUser(accessToken, testOrgId, testUserId);
